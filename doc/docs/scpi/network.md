...
```python
static_ip = inst.query('NET:IPADdress?')   # 192.168.1.50 (skonfigurowany)
actual_ip  = inst.query('NET:STATe?')       # 192.168.1.67 (przydzielony przez DHCP)
```
